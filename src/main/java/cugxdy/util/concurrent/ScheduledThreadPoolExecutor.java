...
import cugxdy.util.concurrent.Delayed;
...
import cugxdy.util.concurrent.RunnableScheduledFuture;
import cugxdy.util.concurrent.ScheduledExecutorService;
import cugxdy.util.concurrent.ScheduledFuture;
...
import cugxdy.util.concurrent.atomic.AtomicLong;
import cugxdy.util.concurrent.locks.Condition;
import cugxdy.util.concurrent.locks.ReentrantLock;

import static cugxdy.util.concurrent.TimeUnit.NANOSECONDS;
...
public class ScheduledThreadPoolExecutor
     ...
     */
	// 在shutdown状态下继续执行周期任务
    private volatile boolean continueExistingPeriodicTasksAfterShutdown;

    /**
     * False if should cancel non-periodic tasks on shutdown.
     */
    // 在shutdown状态下继续执行定时任务
    private volatile boolean executeExistingDelayedTasksAfterShutdown = true;

    /**
     * True if ScheduledFutureTask.cancel should remove from queue
     */
    // 当cancel时,是否删除该定时任务对象
    ...
    private class ScheduledFutureTask<V>
        ...
        public void run() {
            ...
            // 即判断在shutdown下是否去处理定时任务
            if (!canRunInCurrentRunState(periodic))
                cancel(false); // 撤销定时任务
            else if (!periodic) // 定时任务
    ...
    void onShutdown() {
        ...
        // 获取在线程池已shutdown的情况下是否继续执行现有定时任务
        boolean keepDelayed =
            getExecuteExistingDelayedTasksAfterShutdownPolicy();
        
        // 获取在线程池已 shutdown 的情况下是否继续执行现有周期任务
     ...
     */
    // DelayedWorkQueue 延迟队列实现类(基于数组形式) 基于堆的数据结构
    // 优先级队列
