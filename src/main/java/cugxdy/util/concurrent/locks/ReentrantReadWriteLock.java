...
public class ReentrantReadWriteLock
    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
         ...
         */
        // 2^16 = 65536
        static final int SHARED_SHIFT   = 16;  // 16 
        static final int SHARED_UNIT    = (1 << SHARED_SHIFT); // 65536
        // 二进制形式:1111111111111111
        ...
        // 它记录第一个获取读锁的线程持有当前锁的计数（主要用于重入）。
        private transient int firstReaderHoldCount;

        Sync() {
        	// 初始化ThreadLocalHoldCounter对象
         ...
         */
        // 写阻塞
        // 在非公平锁时:返回false,即允许抢占式获取锁,不管同步队列中是否存在等待线程
        // 在公平锁时:判断队列中是否存在等待锁的线程,如果存在返回true,不存在返回false
        ...
        protected final boolean tryRelease(int releases) {
            ...
            // 当写状态为0时,即将锁持有线程设置为null
            if (free)
                setExclusiveOwnerThread(null);
            // 以获取写锁，不需要其他同步措施，是线程安全的
            setState(nextc);
            return free;
        }

        // 尝试获取写锁
        // 如果是以存在读锁时并且第一次获取写锁,那么将会获取失败。就会添加至同步队列中
        // 如果是以存在读锁时并非第一次获取写锁,获取写锁的线程与持有写锁的线程不一致时,将会失败,即会加入至同步队列中返回
        // 如果CAS设置的话,将会获取写锁失败。
        protected final boolean tryAcquire(int acquires) {
            ...
            // 这里存在几种情况:说明一下
            // 1、c!=0:表示存在读写锁,不管是哪个
            // w == 0 : 说明只存在读锁,所以在只有读锁的情况下,是不能获取写锁的,保证内存可见性
            // current = getExclusiveOwnerThread():
            // 这种情况就是,在上层业务逻辑中,存在线程获取写锁,然后在获取读锁,然后写锁(具有可重入性)
            if (c != 0) {
        ...
        protected final boolean tryReleaseShared(int unused) {
            ...
            for (;;) {
                ...
                if (compareAndSetState(c, nextc))
                	...
                	// 这里为什么判断==0,因为当nextc==0时,即同步器锁未被任何线程获取
                	// 因此需要去唤醒同步队列中的线程
                    return nextc == 0;
            }
        }
        ...
        protected final int tryAcquireShared(int unused) {
             ...
             */
        	// 在存在写锁的条件下,只有持有写锁的线程能获取读锁
        	
            Thread current = Thread.currentThread();
            int c = getState();
            
            // exclusiveCount(c)取低16位写锁标志位。
            // 在存在写锁的前提下,只有持有写锁的线程能获取读锁
            ...
            // 除非当前线程的下一个节点被取消，否则fullTryAcquireShared也获取不到读锁
