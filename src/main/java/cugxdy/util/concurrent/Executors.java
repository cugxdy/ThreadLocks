...
import cugxdy.util.concurrent.ForkJoinPool;
...
import cugxdy.util.concurrent.ScheduledThreadPoolExecutor;
...
public class Executors {
     ...
     */
    // SingleThreadExecutor 设置核心池大小和最大池大小都为1,keepAliveTime=0,
    // 无界的等待队列(LinkedBlockingQueue),此时没有指定拒绝策略，默认的拒绝的策略就是AbortPoliy,
     ...
     * @return the newly created thread pool
     */
    // 核心池的大小为0，最大池大小非常大,可以认为是无穷。超时时间是60s,因为这里的核心池大小为0,
    // 所以所有的线程超过60s没事干都会被回收。
    // 等待队列给的是SynchronousQueue,这是一个没有容量的同步队列，所以可以认为所有任务都会被直接提交。
    // 从这些参数中可以看出,如果同时有大量任务提交,而这些任务的执行又不那么快的话,系统会开启等量线程来处理,
    // 这样的做法会快速耗尽系统资源。
